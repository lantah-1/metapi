...
function createInspector(
  ...
  options?: {
    ...
    existingTables?: Array<'token_routes' | 'route_channels' | 'route_group_sources' | 'route_header_templates'>;
  },
) {
  ...
  const inspector: RouteGroupingSchemaInspector = {
    ...
    async tableExists(table) {
      return existingTables.has(table as 'token_routes' | 'route_channels' | 'route_group_sources' | 'route_header_templates');
    },
  ...
  it.each([
    {
      ...
      expectedSql: [
        ...
        'ALTER TABLE token_routes ADD COLUMN custom_header_template_id integer REFERENCES route_header_templates(id) ON DELETE set null;',
        'ALTER TABLE token_routes ADD COLUMN custom_headers text;',
    ...
    },
    {
      ...
      expectedSql: [
        ...
        'ALTER TABLE `token_routes` ADD COLUMN `custom_header_template_id` INT NULL',
        'ALTER TABLE `token_routes` ADD COLUMN `custom_headers` TEXT NULL',
  ...
  it('skips existing columns', async () => {
    const { inspector, executedSql } = createInspector('postgres', {
      existingColumnsByTable: {
        token_routes: ['display_name', 'display_icon', 'route_mode', 'decision_snapshot', 'decision_refreshed_at', 'routing_strategy', 'custom_header_template_id', 'custom_headers'],
        route_channels: ['source_model', 'last_selected_at', 'consecutive_fail_count', 'cooldown_level'],
      },
      existingTables: ['token_routes', 'route_channels', 'route_group_sources', 'route_header_templates'],
    });
