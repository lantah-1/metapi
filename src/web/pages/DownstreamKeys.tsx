...
function buildEditorForm(item?: ManagedItem | DownstreamApiKeyItem | null): DownstreamKeyEditorForm {
  return {
    name: item?.name || '',
    key: item?.key || '',
...
export default function DownstreamKeys() {
  ...
  }), [activeTagFilters, groupFilter, managedItems, searchMatcher, status, tagMatchMode]);
    ...
    <div className="animate-fade-in" style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div className="page-header" style={{ marginBottom: 0 }}>
        <div>
          ...
          <div className="page-subtitle">统一管理分发给下游项目的密钥、主分组、标签、额度、有效期与历史用量。</div>
        </div>
      ...
      >
        <div className="info-tip" style={{ marginBottom: 0 }}>
          本次会对已选中的 {selectedIds.length} 个密钥批量设置主分组，并追加标签。不会改动额度和有效期。
        </div>
